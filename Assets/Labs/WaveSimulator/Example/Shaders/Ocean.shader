...
Shader "Scarecrow/Ocean"
{
    Properties
    {
        ...
        _OceanColorFre ("Ocean Color Fre", Color) = (1, 1, 1, 1)
        ...
        _ShoreRange ("Shore Range", Range(0,1)) = .5
        _EdgeRange ("Edge Range", Range(0,1)) = .5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 CalculateSSSColor(float3 lightDirection, float3 worldNormal, float3 viewDir,float waveHeight, float shadowFactor)
            {
                    ...
                    return _OceanColorSSS * (SSSFactor + waveHeight * 0.6);
            }
            ...
            fixed4 frag(v2f i): SV_Target
            {
                ...
                depth = 1 - Linear01Depth(depth)*(1/_ShoreRange);
                ...
                fixed bubblesControl = saturate(dot(lightDir, normal)) * 1.3;
                ...
                fixed specularControl = pow(max(0, dot(normal, halfDir)), _Gloss) < 0.2 ? 0.1 : 0.3;
                specular *= specularControl;
                specular *= sky * .3;
                fixed3 diffuse = lerp(oceanDiffuse, bubblesDiffuse, bubbles);

                //折射
                fixed4 fra = tex2D(_ScreenTex, (i.screenUV.xy + 1.05 * sin(2 * _Time.y))/i.screenUV.w);

                fixed4 sss = CalculateSSSColor(lightDir,normal,viewDir,i.worldPos.y + _SSSHeight,.8);
                fixed3 col = ambient + lerp(diffuse, _OceanColorFre, fresnel) + specular ;
                col +=  _CausticsIntensity * caustics;
                col = (1-sss.a)*col+sss.a*sss;
                col = lerp(fra,col,1-depth);
