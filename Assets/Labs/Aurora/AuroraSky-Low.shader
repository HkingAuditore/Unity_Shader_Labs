Shader "Aurora/AuroraSky-Low"
{
    ...
    float4 SurAurora(float3 pos,float3 ro)
    {
        ...
        {
            // 坐标
            float of = 0.006*SurHash(pos.xy)*smoothstep(0,15, i*3);       
            float pt = ((0.8+pow(i*3,1.4)*0.002)-ro.y)/(pos.y*2.0+0.8);
            pt -= of;
            ...
            col2.rgb = (sin(1.0-float3(2.15,-.5, 1.2)+i*3*_SurAuroraColFactor*0.1)*0.8+0.5)*noise*3;
            avgCol =  lerp(avgCol, col2, 0.5);
            col += avgCol*exp2(-i*3*0.065 - 2.5)*smoothstep(0.,5., i*3)*1.5;
